...
const updateUserInfo = (req, res, next) => {
    const {id, username, nickname, avatarUrl, phone, email, gender, birthday} = req.body;
    usersService.updateUserInfo(id, username, nickname, avatarUrl, phone, email, gender, dateFunction(birthday), (err, result) => {
        if (err) {
            return res.send({state: 1, message: err});
        }
        return res.send({state: 0, message: "修改成功", data: result});
